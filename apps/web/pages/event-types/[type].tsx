...
const EventTypePage = (props: inferSSRProps<typeof getServerSideProps>) => {
  ...
  const updateMutation = trpc.useMutation("viewer.eventTypes.update", {
    onSuccess: async ({ eventType }) => {
      window.parent.postMessage({ message: "closePopup", value: eventType }, "*");
      console.log('posted');
      showToast(
                  ...
                  <div className="mt-4 flex justify-end space-x-2 rtl:space-x-reverse">
                    <Button type="button" onClick={
                      ()=> {
                       ...
                       } className={classNames('text-techiepurple bg-transparent hover:drop-shadow-none')} tabIndex={-1}>
                      {t("cancel")}
                    </Button>
                    <Button type="submit" data-testid="update-eventtype" className={classNames('bg-techiepurple rounded-md')} disabled={updateMutation.isLoading}>
                      {t("update")}
                    </Button>
                  </div>
                </Form>
